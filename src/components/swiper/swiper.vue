<template>
  <div class="vux-slider">
    <div class="vux-swiper" :style="{height: xheight}">
      ...
      <div v-if="listTwoLoopItem.length > 0" class="vux-swiper-item vux-swiper-item-clone"
           v-for="(item, index) in listTwoLoopItem" @click="clickListItem(item)" :data-index="index">
        <a href="javascript:">
          <div class="vux-img" :style="{backgroundImage: buildBackgroundUrl(item.img)}"></div>
          <p class="vux-swiper-desc" v-if="showDescMask">{{item.title}}</p>
        </a>
...
<script>
  ...
  let getUrl = function (url, $router) {
